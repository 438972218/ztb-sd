      ...
      >
        ...
        <a-row>
          ...
          <a-col :sm="24" :md="12" :xl="12">
            ...
            >
              <a-select
                v-model="form.scoringFormula"
                @change="bankChange"
                placeholder="请选择计分方式"
                :disabled="disableds"
              >
                <a-select-option value="平均分">平均分</a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
        </a-row>
...
<script>
import {
  vendorKpiPostRequestAll,
  vendorKpiShowGetRequest,

  vendorKpiwithDetailPutRequestAll,
  ...
  methods: {
    ...
    getShowAll (id) {
      vendorKpiShowGetRequest(id).then((response) => {
        ...
        this.dataSource = response.data.vendorKpiDetailVOS;
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          vendorKpiPostRequestAll({
            ...
            scoringFormula: this.form.scoringFormula,
            vendorKpiDetailDTOS: list,
          }).then((response) => {
            if (response.code === 0) {

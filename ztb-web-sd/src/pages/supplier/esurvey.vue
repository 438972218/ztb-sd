      ...
      <div>
        ...
        >
          ...
          >
            <div :key="col">
              <template>
               {{ $moment(Number(record.vendorVO[col])).format("YYYY-MM-DD") }}
              </template>
            </div>
          </template>
          <template
...
<script>
import {
  vendorQuestionGetRequest,
  vendorQuestionDeleteRequest,
  ...
  methods: {
    ...
    c_delete(id) {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk() {
          vendorQuestionDeleteRequest(id).then((response) => {
            if (response.code === 0) {

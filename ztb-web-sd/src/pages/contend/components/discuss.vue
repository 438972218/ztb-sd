<template>
  <div>
    <a-row
      ...
      v-if="project.requestStatusName !== '截止' && !getUserInfo"
    >
      <a-col :span="24">
        ...
        >
          ...
          >
            ...
            >
              ...
              <div v-else>
                <a-space>
                  <a-button type="primary" @click="handleAdd(row.subject)">
                    发表评论
                  </a-button>
            ...
            <a-list-item slot="renderItem" slot-scope="item">
              <a
                slot="actions"
                v-if="item.from.account !== user.userName"
                @click="handleReply(row.id, item.from)"
                >回复</a
              >
            ...
            <a-form-model-item label="讨论人" prop="toUsers">
              ...
              >
                ...
                <a-select-option
                  ...
                  :key="item.vendorUserId"
                  :value="item.vendorUserName"
                  >{{ item.linkman }}</a-select-option
                >
              </a-select>
            </a-form-model-item>
          </a-col>
...
export default {
  ...
  methods: {
    ...
    handleChange (e) {
      ...
      if (data) {
        this.form.toUsers = this.userList.map(item => item.vendorUserName)
      } else {
        this.form.toUsers = e
      }
    },
    ...
    async saveReply () {
      if (this.replyForm.toUsers && this.replyForm.toUsers.length === 0) {
        this.userList.forEach(item => {
          this.replyForm.toUsers.push(item.vendorUserName)
        })
      }
    ...
    // 创建讨论
    handleAdd (name) {
      this.subjectAdd = name ? true : false
      this.form = {
        content: '', // 内容
        requestId: this.project.requestId, // 表单ID
        subject: name ? name : '', // 主题
        toUsers: [] // 收信人
      }
      this.toUsers = []
      this.visible = true

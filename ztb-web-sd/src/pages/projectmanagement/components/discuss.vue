...
import { getProjectMemberPageByCondition, postDiscussInitiate, getDiscussGroup, getDiscussRecordRequest, putDiscussReply } from "@/services/projectmanagement/project";
export default {
  props: {
    project: {
      type: [Object, String],
  ...
  methods: {
    ...
    async getDiscussRecord () {
      const result = await getDiscussRecordRequest(this.project.id)
      if (result.code === 0) {
    ...
    handleAdd () {
      this.form = {
        content: '', // 内容
        requestId: this.project.id, // 表单ID
        subject: '', // 主题
        toUsers: [] // 收信人
      }
      this.toUsers = []

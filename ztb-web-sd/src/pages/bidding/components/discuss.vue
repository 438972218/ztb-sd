<template>
  <div>
    <a-row
      ...
      v-if="project.status !== '已完成' && !getUserInfo"
    >
      <a-col :span="24">
    ...
    >
      ...
      >
        <a-row :gutter="20">
          ...
          <a-col :span="24">
            <a-form-model-item label="讨论人" prop="toUsers">
              <a-select
                mode="multiple"
                allowClear
                v-model="toUsers"
              ...
              >
                ...
                <a-select-option
                  ...
                  :key="item.sysUserInfoVO.id"
                  :value="item.sysUserInfoVO.userName"
                  >{{ item.sysUserInfoVO.name }}</a-select-option
                >
              </a-select>
            </a-form-model-item>
          </a-col>
...
import { postDiscussInitiate, getDiscussGroup, getDiscussRecordRequest, putDiscussReply } from "@/services/projectmanagement/project";
import { bidVendorGetwithUser } from "@/services/source/bidding";
export default {
  name: "discuss",
  props: {
    getUserInfo: {
  ...
  methods: {
    ...
    // 选择讨论人
    handleChange (e) {
      const data = e.find(f => f === -1)
      if (data) {
        this.form.toUsers = this.userList.map(item => item.sysUserInfoVO.userName)
      } else {
        this.form.toUsers = e
      }
    },
    ...
    async saveReply () {
      if (this.replyForm.toUsers && this.replyForm.toUsers.length === 0) {
        this.userList.forEach(item => {
          this.replyForm.toUsers.push(item.sysUserInfoVO.userName)
        })
      }
    ...
    handleReply (id, row) {
      this.replyForm = {
        ...
        toUsers: row.account ? [row.account] : []
      }
    ...
    async getDataList () {
      ...
      if (result.code === 0) {
        ...
        this.activeKey = this.data ? (this.data.length > 0 ? this.data[0].id : '') : ''
        this.getDiscussRecord()
      } else {
        this.$message.error(result.message)
      }
    ...
    async getVendorMember () {
      const data = {
        bidSheetId: this.project.id,
        currentPage: -1
      }
      const result = await bidVendorGetwithUser(data)
      if (result.code === 0) {
        this.userList = result.data ? result.data.records : []
      } else {
        this.$message.error(result.message)
      }
    },

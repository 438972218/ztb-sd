    ...
    <a-row>
      <a-table :columns="columns1" :data-source="data1" :scroll="{ x: 1300 }">
        ...
        >
          ...
          <span v-if="record.paidVendorVO" > {{record.paidVendorVO.vendorName}} </span>
        </template>
      <template slot="linkman" slot-scope="text, record">
          <span v-if="record.paidVendorVO" > {{record.paidVendorVO.linkman}} </span>
        </template>
...
// 请求物品
    ...
    // 请求物品竞价paidVendorMaterialNewPricePostRequest
    getItemAll(id) {
      paidVendorMaterialNewPricePostRequest({
        materialId: id,
      }).then((response) => {

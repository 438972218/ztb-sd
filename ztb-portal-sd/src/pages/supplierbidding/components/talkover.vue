...
export default {
  ...
  mounted () {
    this.project = JSON.parse(sessionStorage.getItem('contendForm'))
  ...
  methods: {
    ...
    // 选择讨论人
    handleChange (e) {
      const data = e.find(f => f === '所有人')
      if (data) {
        this.form.toUsers = this.userList.map(item => item.name)
      } else {
        this.form.toUsers = e
      }
    },
    ...
    // 回复讨论
    async saveReply () {
      if (this.replyForm.toUsers.length === 0) {

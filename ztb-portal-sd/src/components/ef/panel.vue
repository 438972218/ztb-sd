<template>
  <div v-if="easyFlowVisible" style="height: calc(90vh)">
    <el-row>
      <!--顶部工具菜单-->
      <el-col :span="24">
        <div class="ef-tooltar">
          ...
          <div style="float: right; margin-right: 5px">
            ...
            <el-button
              ...
              plain
              round
              @click="dataReloadAll(item)"
              icon="el-icon-refresh"
              size="mini"
              >{{ item.version }}</el-button
            >
            <el-button
              type="primary"
              plain
    ...
    <div style="display: flex; height: calc(90% - 47px)">
      <div style="width: 175px; border-right: 1px solid #dce3e8">
    ...
    >
      <a-form-model
        ...
        :rules="rules"
      >
        <a-form-model-item prop="title" label="流程模板名称">
          <a-input v-model="form.title" />
        </a-form-model-item>
        <a-row style="text-align: center">
          <a-space>
            <a-button type="primary" @click="titleOk">新建</a-button>
...
import { processDaveRequest } from "@/services/process/workflow";
import { moduleListChange } from "@/utils/methods";
...
export default {
  ...
  watch: {
    moduleList(newVal, oldVal) {
      if (newVal) {
        this.moduleList = newVal;
        if (this.moduleList.length !== 0) {
          this.reloadShow = true;
          this.dataList = moduleListChange(this.moduleList[0]);
          this.jsPlumb = jsPlumb.getInstance();
          this.dataReload(this.dataList);
          // this.$nextTick(() => {
  ...
  methods: {
      ...
      this.jsPlumb.ready(() => {
        ...
        this.jsPlumb.bind("connection", (evt) => {
          ...
          if (this.loadEasyFlowFinish) {
            let label = this.hasLineType(from, to);
            if (label === null) {
        ...
        this.jsPlumb.bind("beforeDrop", (evt) => {
          ...
          if (this.hasLineTypeAmount(from, to)) {
            this.$message.error("条件节点不能多于两条");
            return false;
          }
    ...
    hasLineType(from, to) {
      ...
      for (let i = 0; i < this.data.nodeList.length; i++) {
        ...
        if (node.id === from && node.type === "3") {
          type = "0";
          for (let index = 0; index < this.data.lineList.length; index++) {
            let line = this.data.lineList[index];
            if (line.from === from) {
              type = "1";
            }
          }
    ...
    // 模拟载入数据dataA
    dataReloadAll(data) {
      this.dataReload(moduleListChange(data));
    },
    addDataList() {
      this.titleVisible = true;
    },
    dataShift() {
      ...
      var arry2 = [];
      list.nodeList.map((item, index) => {
        arry2.push(
          Object.assign({}, item, {
            statusMark: item.id,
            condition: { name: item.condition, script: item.script },
            toRoleId: this.getUUID(),
          })
        );
      });
      for (let i = 0; i < list.lineList.length; i++) {
        if (list.lineList[i].label) {
          list.lineList[i].label = Number(list.lineList[i].label);
        }
      }
      for (let i = 0; i < arry2.length; i++) {
        arry2[i].type = Number(arry2[i].type);
        arry2[i].toRoleId = Number(arry2[i].toRoleId);
        if( arry2[i].condition.name === undefined && arry2[i].condition.script === undefined){
        delete arry2[i].condition;
        }
        delete arry2[i].id;
      }
      list.nodeList = arry2;

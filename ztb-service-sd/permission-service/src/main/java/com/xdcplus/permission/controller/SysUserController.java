...
import com.xdcplus.permission.common.pojo.dto.sysuser.SysUserDto;
import com.xdcplus.permission.common.pojo.dto.sysuser.SysUserFilterDto;
import com.xdcplus.permission.common.pojo.dto.sysuser.UpdatePasswordByIdDto;
...
public class SysUserController extends AbstractController {
    ...
    @ApiOperation("忘记密码")
    @PutMapping(value = "forgetPassword/{userName}", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseVO forgetPassword( @PathVariable("userName")
                               @NotNull(message = "userName不能为空") String userName){
        log.info("updatePasswordById:{}", userName);
        sysUserService.forgetPassword(userName);
        return ResponseVO.success();
    }

    @ApiOperation("重置密码(管理员使用)")
    @PutMapping(value = "resetPasswordById/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseVO resetPasswordById( @PathVariable("id")
                               @NotNull(message = "id不能为空") Long id){
        log.info("resetPasswordById:{}", id);
        sysUserService.resetPasswordById(id,AuthUtils.getCurrentUser());
        return ResponseVO.success();
    }

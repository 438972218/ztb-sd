...
public class ApiController extends AbstractController {
    ...
    @GetMapping(value = "queryUserMap", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<Map<String,String>> queryUserMap() {
        return ResponseVO.success(this.sysUserService.queryMap());
    }

    @ApiOperation("根据userName查询出下属的userNames")
    @GetMapping(value = "getSubordinateUserNames/{userName}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<List<String>> getSubordinateUserNames(@PathVariable("userName")
                                               @NotNull(message = "userName不能为空")String userName) {
        return ResponseVO.success(this.sysUserService.getSubordinateUserNames(userName));
    }

    @ApiOperation("根据角色id查询出所有的用户信息")
    @GetMapping(value = "getSysUserByRoleId/{roleId}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<List<SysUserVo>> getSysUserByRoleId(@PathVariable("roleId")
                                                            @NotNull(message = "roleId不能为空")Long roleId) {
        return ResponseVO.success(this.sysUserService.getSysUserByRoleId(roleId));
    }

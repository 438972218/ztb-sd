...
import com.xdcplus.permission.captcha.SpecCaptcha;
import com.xdcplus.permission.captcha.base.Captcha;
import com.xdcplus.permission.common.pojo.dto.auth.Oauth2TokenDTO;
import com.xdcplus.permission.common.pojo.dto.auth.UserLoginDTO;
import com.xdcplus.permission.common.pojo.vo.auth.Oauth2TokenVO;
import com.xdcplus.permission.common.pojo.vo.sysuser.UserPermVO;
import com.xdcplus.permission.constants.Oauth2Constant;
import com.xdcplus.permission.enums.UrlEnum;
import com.xdcplus.permission.security.Oauth2Token;
import com.xdcplus.permission.service.LoginService;
...
//        retryLimit(account, userLoginDTO.getPassword(), userPermVO.getPassword(), userPermVO.getSourceType());
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        String key = AuthConstant.PREFIX_SHIRO_CACHE + AuthUtils.getCurrentUser();
        if (redisUtils.hasKey(key)) {
            redisUtils.delete(AuthConstant.PREFIX_SHIRO_REFRESH_TOKEN + redisUtils.get(key, String.class));
            redisUtils.delete(AuthConstant.PREFIX_SHIRO_CACHE + key);
        }else {
    ...
    private Oauth2TokenVO generateToken(HttpServletResponse response, String account, String password) {
        ...
        Oauth2TokenVO oauth2TokenVO = null;
        try{
        ...
        if (redisUtils.hasKey(AuthConstant.PREFIX_SHIRO_CACHE + account)) {
            redisUtils.delete(AuthConstant.PREFIX_SHIRO_CACHE + account);
        }

        redisUtils.setEx(AuthConstant.PREFIX_SHIRO_CACHE + account, token, NumberConstant.THREE_THOUSAND_AND_SIX_HUNDRED, TimeUnit.SECONDS);

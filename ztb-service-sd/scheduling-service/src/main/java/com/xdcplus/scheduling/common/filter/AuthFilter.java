...
public class AuthFilter implements GlobalFilter, Ordered {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		try {
			...
			if (!redisUtils.hasKey(AuthConstant.PREFIX_SHIRO_CACHE + payload.getAccount())) {
				log.error("Account {} is not logged in", payload.getAccount());
			...
			if (!StrUtil.equals(realToken, redisUtils.get(AuthConstant.PREFIX_SHIRO_CACHE + payload.getAccount(), String.class))) {
				log.error("Account {} has been remotely logged in to. Please log in again", payload.getAccount());
				return WebUtils.writeFailedToResponse(response, ResponseEnum.THE_ACCOUNT_HAS_BEEN_LOGGED_IN_TO_THE_REMOTE_SITE_PLEASE_LOG_IN_AGAIN);
			}
			...
			redisUtils.expire(AuthConstant.PREFIX_SHIRO_REFRESH_TOKEN + realToken, NumberConstant.THREE_THOUSAND_AND_SIX_HUNDRED, TimeUnit.SECONDS);
			redisUtils.expire(AuthConstant.PREFIX_SHIRO_CACHE + payload.getAccount(), NumberConstant.THREE_THOUSAND_AND_SIX_HUNDRED, TimeUnit.SECONDS);

...
public class BidDetailController extends AbstractController {
    ...
    public ResponseVO batchUpdateBidDetail(@RequestBody BidDetailUpdateDTO bidDetailUpdateDTO) {

        log.info("batchUpdateBidDetail {}", bidDetailUpdateDTO.toString());
        if (CollectionUtil.isNotEmpty(bidDetailUpdateDTO.getBidDetailDTOS())) {
            bidDetailUpdateDTO.getBidDetailDTOS().forEach(bidDetailDTO -> {
                bidDetailDTO.setCreatedUser(getAccount());
                bidDetailDTO.setBidSheetId(bidDetailUpdateDTO.getBidSheetId());
                bidDetailDTO.setBidType(bidDetailUpdateDTO.getBidType());
            });
        }

        bidDetailService.batchUpdateBidDetailDTO(bidDetailUpdateDTO);

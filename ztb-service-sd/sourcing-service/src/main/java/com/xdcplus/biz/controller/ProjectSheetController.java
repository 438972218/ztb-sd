...
public class ProjectSheetController extends AbstractController {
    ...
    public ResponseVO<PageVO<ProjectSheetVO>> findProjectSheet(ProjectSheetFilterDTO projectSheetFilterDTO) {
        ...
        return ResponseVO.success(projectSheetService.queryProjectSheetWithCreatedUser(projectSheetFilterDTO));
    }
    ...
    public ResponseVO saveProjectSheetReturnVO(@RequestBody ProjectSheetDTO projectSheetDTO) {
        ...
        projectSheetDTO.setCreatedUser(getAccount());
        projectSheetDTO.setType("official");
    ...
    @ApiOperation("修改项目返回vo")
    @PutMapping(value = "/returnVO", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO updateProjectSheetReturnVO(@RequestBody ProjectSheetDTO projectSheetDTO) {

        log.info("updateProjectSheetReturnVO {}", projectSheetDTO.toString());

        projectSheetDTO.setUpdatedUser(getAccount());
        projectSheetService.updateProjectSheet(projectSheetDTO);

        return ResponseVO.success(projectSheetService.showProjectSheet(projectSheetDTO.getId()));
    }
    ...
    @ApiOperation("发送邮件给项目成员")
    @GetMapping(value = "sendMailByMember/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "path", name = "id", dataType = "Long", value = "id", required = true),
    })
    public ResponseVO sendMailByMember(@PathVariable("id")
                                           @NotNull(message = "ID不能为空") Long id) {

        log.info("sendMailByMember {}", id);

        projectSheetService.sendMailByMember(id);

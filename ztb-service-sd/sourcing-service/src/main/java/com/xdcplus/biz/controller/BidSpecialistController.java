...
public class BidSpecialistController extends AbstractController {
    ...
    @ApiOperation("查询专家withUser")
    @GetMapping(value = "/queryWithUser", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<PageVO<BidSpecialistVO>> queryBidSpecialistWithUser(BidSpecialistFilterDTO bidSpecialistFilterDTO) {

        log.info("queryBidSpecialistWithUser {}", bidSpecialistFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(bidSpecialistFilterDTO);

        return ResponseVO.success(bidSpecialistService.queryBidSpecialistWithUser(bidSpecialistFilterDTO));
    }

    @ApiOperation("新增专家返回VO")
    @PostMapping(value = "/returnVO", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<BidSpecialistVO> saveBidSpecialistReturnVO(@RequestBody BidSpecialistDTO bidSpecialistDTO) {

        log.info("saveBidSpecialistReturnVO {}", bidSpecialistDTO.toString());

        bidSpecialistDTO.setCreatedUser(getAccount());
        ;

        return ResponseVO.success(bidSpecialistService.saveBidSpecialistReturnVO(bidSpecialistDTO));
    }

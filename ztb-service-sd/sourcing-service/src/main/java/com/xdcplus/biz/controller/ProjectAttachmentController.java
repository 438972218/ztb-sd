...
public class ProjectAttachmentController extends AbstractController {
    ...
    @ApiOperation("查询项目附件WithName")
    @GetMapping(value = "/withName", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<PageVO<ProjectAttachmentVO>> findProjectAttachmentWithName(ProjectAttachmentFilterDTO projectAttachmentFilterDTO) {

        log.info("findProjectAttachmentWithName {}", projectAttachmentFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(projectAttachmentFilterDTO);

        return ResponseVO.success(projectAttachmentService.queryProjectAttachmentWithName(projectAttachmentFilterDTO));
    }

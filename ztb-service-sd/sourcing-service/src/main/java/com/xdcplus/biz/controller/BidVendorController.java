...
public class BidVendorController extends AbstractController {
    ...
    public ResponseVO<BidVendorVO> saveBidVendorWithDefault(@RequestBody BidVendorDTO bidVendorDTO) {

        log.info("saveBidVendorWithDefault {}", bidVendorDTO.toString());

        bidVendorDTO.setCreatedUser(getAccount());

        return ResponseVO.success(bidVendorService.saveBidVendorWithDefaultReturnVO(bidVendorDTO));
    }

    @ApiOperation("新增招标投标方withDetail(正在运行)")
    @PostMapping(value = "/withDetail", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<BidVendorVO> saveBidVendorWithDetail(@RequestBody BidVendorDTO bidVendorDTO) {

        log.info("saveBidVendorWithDetail {}", bidVendorDTO.toString());

        bidVendorDTO.setCreatedUser(getAccount());

        return ResponseVO.success(bidVendorService.saveBidVendorWithDetailReturnVO(bidVendorDTO));
    }
    ...
    @ApiOperation("查询招标投标方WithVendor")
    @GetMapping(value = "/withVendor", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<PageVO<BidVendorVO>> findBidVendorWithVendor(BidVendorFilterDTO bidVendorFilterDTO) {

        log.info("queryBidVendorWithVendor {}", bidVendorFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(bidVendorFilterDTO);

        return ResponseVO.success(bidVendorService.queryBidVendorWithVendor(bidVendorFilterDTO));
    }

    @ApiOperation("招标放弃授权")
    @GetMapping(value = "/waiveAuth/{bidSheetId}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiImplicitParams({
        @ApiImplicitParam(paramType = "path", name = "bidSheetId", dataType = "Long", value = "招标单ID", required = true),
    })
    public ResponseVO<PageVO<BidVendorVO>> waiveAuth(@PathVariable("bidSheetId")
                                                     @NotNull(message = "招标单ID不能为空") Long bidSheetId) {

        log.info("waiveAuth {}", bidSheetId);

        return ResponseVO.success(bidVendorService.waiveAuth(bidSheetId));
    }

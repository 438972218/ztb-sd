...
public class DictionaryController extends AbstractController {
    ...
    @ApiOperation("根据class和numeerial查询字典管理")
    @GetMapping(value = "/selectByClassAndNumerial", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<DictionaryVO> findDictionaryVOByClassAndNumerical(DictionaryFilterDTO dictionaryFilterDTO) {

        log.info("findDictionaryVOByClassAndNumerical {}", dictionaryFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(dictionaryFilterDTO);

        return ResponseVO.success(dictionaryService.findDictionaryVOByClassAndNumerical(dictionaryFilterDTO.getDictionaryClass(),dictionaryFilterDTO.getNumerical()));
    }

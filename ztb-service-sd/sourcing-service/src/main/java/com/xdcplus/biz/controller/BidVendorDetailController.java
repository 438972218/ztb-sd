...
public class BidVendorDetailController extends AbstractController {
    ...
    @ApiOperation("查询供应商内容明细(按轮次分组)")
    @GetMapping(value = "/queryRoundMap", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<List<BidVendorDetailMapVO>> queryBidVendorDetailMap(BidVendorDetailFilterDTO bidVendorDetailFilterDTO) {

        log.info("queryBidVendorDetailMap {}", bidVendorDetailFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(bidVendorDetailFilterDTO);

        return ResponseVO.success(bidVendorDetailService.queryBidVendorDetailMap(bidVendorDetailFilterDTO));
    }

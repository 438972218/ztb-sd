...
public class PaidAttachmentController extends AbstractController {
    ...
    @ApiOperation("查询竞价单附件withUser")
    @GetMapping(value = "/withUser", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<PageVO<PaidAttachmentVO>> queryPaidAttachmentWithUser(PaidAttachmentFilterDTO paidAttachmentFilterDTO) {

        log.info("queryPaidAttachmentWithUser {}", paidAttachmentFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(paidAttachmentFilterDTO);

        return ResponseVO.success(paidAttachmentService.queryPaidAttachmentWithUser(paidAttachmentFilterDTO));
    }

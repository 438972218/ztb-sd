package com.xdcplus.statement.remote;

import com.xdcplus.statement.remote.fallback.PermRemoteFallbackFactory;
...
public interface PermRemote {
    ...
    /**根据userName查询出下属的userNames
     *
     * @param userName
     * @return
     */
    @GetMapping(value = "getSubordinateUserNames/{userName}", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseVO<List<String>> getSubordinateUserNames(@PathVariable("userName") String userName);

    /**
     * 根据角色id查询出所有的用户信息
     *
     * @param roleId 角色id
     * @return {@link ResponseVO<List<SysUserInfoVO>>}
     */
    @GetMapping(value = "getSysUserByRoleId/{roleId}", produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseVO<List<SysUserInfoVO>> getSysUserByRoleId(@PathVariable("roleId") Long roleId);

...
CREATE TABLE IF NOT EXISTS `xdc_t_offer`  (
    ...
    `available` tinyint(4) NULL DEFAULT NULL COMMENT '是否有效, 0:无效，1：有效',
    `trigger_threshold` tinyint(4) NULL DEFAULT NULL COMMENT '是否触发阈值，0:否，1：有',
...
CREATE TABLE IF NOT EXISTS `xdc_t_bulletin_board`  (
    ...
    `type` tinyint(4) NULL DEFAULT NULL COMMENT '公告类型(1->全局公告，2->表单公告)',
    `request_id` bigint(20) NULL DEFAULT NULL COMMENT '表单ID(全局公告->-1)',
    `message` varchar(500) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '内容',

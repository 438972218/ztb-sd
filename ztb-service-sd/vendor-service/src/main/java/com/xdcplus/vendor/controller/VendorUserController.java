...
public class VendorUserController extends AbstractController {
    ...
    @ApiOperation("查询供应商用户WithVendor")
    @GetMapping(value = "/withVendor", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<List<VendorUserVO>> findVendorUserWithVendor(VendorUserFilterDTO vendorUserFilterDTO) {

        log.info("findVendorUserWithVendor {}", vendorUserFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(vendorUserFilterDTO);

        return ResponseVO.success(vendorUserService.findVendorUserWithVendor(vendorUserFilterDTO));
    }

    @ApiOperation("查询供应商用户PageVO")
    @GetMapping(value = "/withVendorPageVO", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<PageVO<VendorUserVO>> queryVendorUserPageVO(VendorUserFilterDTO vendorUserFilterDTO) {

        log.info("queryVendorUserPageVO {}", vendorUserFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(vendorUserFilterDTO);

        return ResponseVO.success(vendorUserService.queryVendorUserPageVO(vendorUserFilterDTO));
    }

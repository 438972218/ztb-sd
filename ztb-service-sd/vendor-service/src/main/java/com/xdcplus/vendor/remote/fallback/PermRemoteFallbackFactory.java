...
public class PermRemoteFallbackFactory implements FallbackFactory<PermRemote> {
    ...
    public PermRemote create(Throwable cause) {
        return new PermRemote() {
            ...
            @Override
            public ResponseVO<SysUserInfoVO> registerUserNotSendEmail(RegisterUserDTO registerUserDto) {
                log.error("registerUserNotSendEmail {}", cause.getMessage());
                return ResponseVO.error();
            }

            @Override
            public ResponseVO<SysUserInfoVO> registerUserCover(RegisterUserDTO registerUserDto) {
                log.error("registerUserCover {}", cause.getMessage());
                return ResponseVO.error();
            }

            @Override
            public ResponseVO<SysUserInfoVO> registerUserNotSendEmailAndCover(RegisterUserDTO registerUserDto) {
                log.error("registerUserNotSendEmailAndCover {}", cause.getMessage());
                return ResponseVO.error();
            }
            ...
            @Override
            public ResponseVO<List<SysUserInfoVO>> queryAllVendorUser() {
                log.error("queryAllVendorUser {}", cause.getMessage());
                return ResponseVO.error();
            }

            @Override
            public ResponseVO<PageVO<SysUserInfoVO>> querySysUserVOPageVO(SysUserInfoDTO sysUserInfoDTO) {
                log.error("querySysUserVOPageVO {}", cause.getMessage());
                return ResponseVO.error();
            }
        };
    }

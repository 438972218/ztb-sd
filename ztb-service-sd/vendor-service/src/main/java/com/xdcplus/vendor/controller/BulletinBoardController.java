...
public class BulletinBoardController extends AbstractController {
    ...
    public ResponseVO<BulletinBoardVO> findLatestAnnouncementByRequestId(@PathVariable("requestId")
                                                                             @NotNull(message = "表单ID 不能为空")
                                                                                     Long requestId) {

        log.info("findLatestAnnouncementByRequestId {}", requestId);

        return ResponseVO.success(bulletinBoardService.findLatestAnnouncementByRequestId(requestId));
    }

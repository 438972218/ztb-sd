...
//        requestFilterDTO.setStatusId(ZtbConstant.RUNNING_STATUS_ID);
        requestFilterDTO.setStatusIds(CollectionUtil.newArrayList(ZtbConstant.RUNNING_STATUS_ID,ZtbConstant.ARCHIVED_STATUS_ID));
        requestFilterDTO.setCurrentPage(-1);
        ...
        for (BidSheet bidSheet : bidSheetList) {
            ...
            BidVendor bidVendor = bidVendors.get(NumberConstant.ZERO);
            bidSheetVO.setVendorStatus(bidVendor.getVendorStatus());
            bidSheetVO.setBidVendorId(bidVendor.getId());
    ...
    public BidSheetVO showBidSheetById(Long id) {
        BidSheetVO bidSheetVO = queryBidSheetById(id);

...
public class VendorController extends AbstractController {
    ...
    @ApiOperation("查询供应商WithUser")
    @GetMapping(value = "/withUser", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<List<VendorVO>> queryVendorsWithUser(VendorFilterDTO vendorFilterDTO) {

        log.info("queryVendorsWithUser {}", vendorFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(vendorFilterDTO);

        return ResponseVO.success(vendorService.queryVendorsWithUser(vendorFilterDTO));
    }

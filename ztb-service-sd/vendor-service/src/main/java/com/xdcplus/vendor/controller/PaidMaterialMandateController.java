...
public class PaidMaterialMandateController extends AbstractController {
    ...
    @ApiOperation("新增竞价品项授权记录WithUser")
    @PostMapping(value = "/saveWithUser", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO savePaidMaterialMandateWithUser(@RequestBody PaidMaterialMandateDTO paidMaterialMandateDTO) {

        log.info("savePaidMaterialMandateWithUser {}", paidMaterialMandateDTO.toString());

        paidMaterialMandateDTO.setCreatedUser(getAccount());
        paidMaterialMandateService.savePaidMaterialMandateWithUser(paidMaterialMandateDTO);

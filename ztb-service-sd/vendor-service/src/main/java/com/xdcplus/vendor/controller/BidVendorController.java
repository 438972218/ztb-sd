...
public class BidVendorController extends AbstractController {
    ...
    @ApiOperation("查询招标投标方WithUser")
    @GetMapping(value = "/withUser", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<PageVO<BidVendorVO>> findBidVendorWithUser(BidVendorFilterDTO bidVendorFilterDTO) {

        log.info("findBidVendorWithUser {}", bidVendorFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(bidVendorFilterDTO);

        return ResponseVO.success(bidVendorService.queryBidVendorWithUser(bidVendorFilterDTO));
    }

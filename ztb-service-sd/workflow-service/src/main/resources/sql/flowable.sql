...
CREATE TABLE IF NOT EXISTS `xdc_t_odd_rule`  (
  ...
  `auto_number` bigint(20) NOT NULL DEFAULT 0 COMMENT '自增长数（自增长算法有效）',
  `created_user` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '添加人',
  `updated_user` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '修改人',
  `created_time` bigint(20) NULL DEFAULT NULL COMMENT '添加时间',
  `updated_time` bigint(20) NULL DEFAULT NULL COMMENT '修改时间',
...
CREATE TABLE IF NOT EXISTS `xdc_t_process_config`  (
  ...
  `user_to` int(10) NULL DEFAULT NULL COMMENT '用户去向标识',
...
CREATE TABLE  IF NOT EXISTS `xdc_t_user_to`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户去向信息' ROW_FORMAT = Dynamic;
...
INSERT IGNORE INTO `xdc_t_user_to` VALUES (6, 6, NULL, NULL, NULL, NULL, NULL, '自定义去向');
...
-- ----------------------------
-- Table structure for xdc_t_user_to_custom_relation
-- ----------------------------
# DROP TABLE IF EXISTS `xdc_t_user_to_custom_relation`;
CREATE TABLE IF NOT EXISTS `xdc_t_user_to_custom_relation`  (
  `id` bigint(20) NOT NULL COMMENT '主键ID',
  `user_to_mark` int(10) NULL DEFAULT NULL COMMENT '用户去向标识',
  `config_id` bigint(20) NULL DEFAULT NULL COMMENT '流程配置ID',
  `expression` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '表达式',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `idx_config_id`(`config_id`) USING BTREE COMMENT '流程配置ID索引',
  INDEX `idx_user_to_mark`(`user_to_mark`) USING BTREE COMMENT '用户去向标识索引'
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '自定义用户去向信息关联表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Table structure for xdc_t_discuss_group_relation
-- ----------------------------
# DROP TABLE IF EXISTS `xdc_t_discuss_group_relation`;
CREATE TABLE IF NOT EXISTS `xdc_t_discuss_group_relation`  (
    `id` bigint(20) NOT NULL COMMENT '主键ID',
    `group_id` bigint(20) NULL DEFAULT NULL COMMENT '讨论组ID',
    `to_user` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '用户账号',
    PRIMARY KEY (`id`) USING BTREE,
    INDEX `idx_group_id`(`group_id`) USING BTREE COMMENT '讨论组ID索引'
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '讨论组关联信息' ROW_FORMAT = Dynamic;

...
public class ProcessConfigControlServiceImpl implements ProcessConfigControlService {
    ...
    @Autowired
    private UserToCustomRelationService userToCustomRelationService;
    ...
    public Boolean saveProcessConfig(List<ProcessConfigTableDTO> processConfigTables) {
        ...
        List<ProcessConfigHolder> configHolderList = refreshConfig(CollectionUtil.newArrayList(processIds).get(NumberConstant.ZERO), processConfigTables);

        saveProcessConfigs(configHolderList);
    ...
    public Boolean saveProcessConfig(ProcessConfigTreeDTO processConfigTreeDTO) {
        ...
        // 转换对象
        List<ProcessConfigHolder> processConfigHolders = refreshConfig(processId, version, configNodes, configLines);

        saveLine(processId, version, configLines);
        saveNode(processId, version, configNodes);

        saveProcessConfigs(processConfigHolders);
     ...
     * 保存流程配置
     *
     * @param configHolderList 流程配置列表
     */
    private void saveProcessConfigs(List<ProcessConfigHolder> configHolderList) {

        if (CollectionUtil.isNotEmpty(configHolderList)) {
            for (ProcessConfigHolder processConfigHolder : configHolderList) {
                ProcessConfig processConfig = BeanUtil.copyProperties(processConfigHolder, ProcessConfig.class);
                processConfigService.save(processConfig);
                if (StrUtil.isNotBlank(processConfigHolder.getUserToExpression())) {
                    Integer userToMark = processConfigHolder.getUserTo();
                    String expression = processConfigHolder.getUserToExpression();
                    if (Validator.equal(NumberConstant.SIX, userToMark)) {
                        expression = ToUserDTO.getToUserNameBySort(Convert.toInt(expression));
                    }
                    userToCustomRelationService.saveRelation(userToMark, processConfig.getId(), expression);
                }
            }
        }
    }
     ...
     * 对象组合
     *
     * @param configTableHolderList 流程配置信息
     * @return {@link List<ProcessConfig>} 流程实体对象
     */
    private List<ProcessConfig> combination(List<ProcessConfigHolder> configTableHolderList) {
        return configTableHolderList.stream()
                .map(a -> {
     ...
     * @return {@link List<ProcessConfigHolder>}
     */
    private List<ProcessConfigHolder> refreshConfig(Long processId, List<ProcessConfigTableDTO> processConfigTables) {

        String version = versionGetAndIncrement(processId);

        return processConfigTables.stream()
                .map(a -> {
                    ProcessConfigHolder processConfigHolder = new ProcessConfigHolder();
                    BeanUtil.copyProperties(a, processConfigHolder);
                    processConfigHolder.setVersion(version);
                    processConfigHolder.setFromStatusId(getProcessStatusIdByMark(a.getFromStatusMask()));
                    processConfigHolder.setToStatusId(getProcessStatusIdByMark(a.getToStatusMask()));
                    ...
                    if (StrUtil.isNotBlank(expression.getExpression())) {
                        ...
                        processConfigHolder.setQualifierId(qualifierService.saveQualifier(qualifierDTO));
                    }
                    return processConfigHolder;
                }).collect(Collectors.toList());
    }
     ...
     */
    private void refreshNodeLine(List<ProcessConfigNodeDTO> configNodes, List<ProcessConfigLineDTO> configLines) {
     ...
     * @param configNodes 配置节点
     * @param configLines 配置线
     * @return {@link List<ProcessConfigHolder>} 流程配置集合
     */
    private List<ProcessConfigHolder> refreshConfig(Long processId, String version, List<ProcessConfigNodeDTO> configNodes, List<ProcessConfigLineDTO> configLines) {
        ...
        return configLines.stream().map(line -> {
            ...
            ProcessConfigHolder processConfig = new ProcessConfigHolder();
            processConfig.setUserToExpression(processConfigNodeDTO.getUserToExpression());
     ...
     */
    @Data
    private static class ProcessConfigHolder implements Serializable {
         ...
         *  自定义用户去向表达式
         */
        private String userToExpression;

...
public class RequestFlowController extends AbstractController {
    ...
    @GetMapping(value = "/transferNode", produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation("根据表单ID和节点状态ID查询流转信息")
    public ResponseVO<List<RequestFlowVO>> findRequestFlowTransferNode(RequestFlowTransferNodeFilterDTO filterDTO) {

        log.info("findRequestFlowTransferNode {}", filterDTO);

        ValidationUtil.validate(filterDTO);

        return ResponseVO.success(requestFlowService.findRequestFlowTransferNode(filterDTO));
    }

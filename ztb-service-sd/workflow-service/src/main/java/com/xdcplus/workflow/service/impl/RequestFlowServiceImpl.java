...
public class RequestFlowServiceImpl extends BaseServiceImpl<RequestFlowBO, RequestFlowVO, RequestFlow, RequestFlowMapper> implements RequestFlowService {
    ...
    @Override
    public List<RequestFlowVO> findRequestFlowTransferNode(RequestFlowTransferNodeFilterDTO filterDTO) {
        List<RequestFlowBO> requestFlowBOList = requestFlowMapper.findRequestFlowTransferNode(filterDTO.getRequestId(), filterDTO.getFromStatusId(), filterDTO.getToStatusId());
        return this.objectConversion(requestFlowBOList);
    }

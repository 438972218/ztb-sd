...
public class RequestController {
    ...
    public ResponseVO<PageVO<RequestVO>> findRequest(RequestFilterDTO requestFilterDTO) {
        ...
        log.debug("findRequest {}", requestFilterDTO);
    ...
    @ApiOperation("查询表单信息根据RequestId列表或创建人列表createdUsers")
    @PostMapping(value = "/findRequest2", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseVO<List<Long>> findRequestIdsByRequestIdsOrCreatedUsers(@RequestBody RequestFilterDTO requestFilterDTO) {

        log.info("findRequest {}", requestFilterDTO);

        Validation.buildDefaultValidatorFactory().getValidator().validate(requestFilterDTO);

        return ResponseVO.success(requestService.findRequestIdsByRequestIdsOrCreatedUsers(requestFilterDTO));
    }

...
public class AgreeProcessTransfor extends BaseProcessTransfor {
    ...
    private class GetDestinationProcessHandler extends BaseAgreeProcessHandler {
        ...
        public void handler(AgreeRequest request) {
            ...
            // 当前流转去向流程配置
            List<ProcessConfigVO> currentConfigList = findConfigByProcessIdAndFromStatusId(requestVO.getProcess().getId(), requestVO.getId(),
    ...
    private class FilterDirectionFlowHandler extends BaseAgreeProcessHandler {
        ...
        public void handler(AgreeRequest request) {
            ...
            // 不是加签
            if (!extraNode) {
                Optional.ofNullable(getsConfigNonAddedDestination(currentConfigList, request.getAgree().getFlowConditions())).ifPresent(nextConfigList::addAll);
            }else {
                ...
                if (equal(NumberConstant.SIX, currentFlow.getToStatus().getMark())) {
                    ...
                    nextConfigList = findConfigByProcessIdAndFromStatusId(currentFlow.getProcess().getId(), request.getRequest().getId(),
